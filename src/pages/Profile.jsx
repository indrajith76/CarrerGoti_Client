...
export default function Profile() {
  ...
  // Experience
  const [experience, setExperience] = useState({ jobTitle: "", companyName: "", duration: "", jobDesc: "" });
  ...
  return (
    <div className="my-5">
      ...
      <div className="flex items-center justify-end gap-5 px-5">
        <button className="btn btn-primary" onClick={() => document.getElementById("profile_modal_1").showModal()}>Upload Your CV</button>
        ...
        <dialog id="profile_modal_1" className="modal">
          <div className="modal-box">
            ...
            <div className="flex justify-center mt-4 mb-6 gap-3">
              <button className={`btn ${mode === "pdf" ? "btn-primary" : "btn-outline"}`} onClick={() => setMode("pdf")}>PDF Upload</button>
              <button className={`btn ${mode === "text" ? "btn-primary" : "btn-outline"}`} onClick={() => setMode("text")}>Text Input</button>
            </div>

            {mode === "pdf" && (
              <div className="my-5 flex w-full gap-3 items-center">
                <input type="file" accept="application/pdf" className="file-input file-input-bordered file-input-primary w-full" />
            ...
            {mode === "text" && (
              <div>
                ...
                <div className="my-5 flex gap-3 items-center">
                  <textarea className="textarea textarea-bordered w-full h-40" placeholder="Paste Your CV Text Content"></textarea>
      ...
      <div className="md:flex flex-col items-center mx-auto mt-5 px-5 py-10">
        <img className="w-28 mx-auto rounded-full" src={user?.image || "https://www.cielhr.com/wp-content/uploads/2020/10/dummy-image.jpg"} />
        ...
        <div className="md:grid md:grid-cols-2 lg:grid-cols-3 gap-x-5">
          ...
          {[
            ...
            { label: "Preferred Career Track", value: user?.preferredCareerTrack },
          ].map((item, i) => (
          ...
          <div className="my-5 md:col-span-3">
            ...
            <div className="flex gap-2">
              <input className="input w-full" value={skill} onChange={(e) => setSkill(e.target.value)} />
          ...
          {skills.length > 0 && (
            <div className="md:col-span-3 my-3">
              {skills.map((s, i) => (
                <span key={i} className="badge badge-outline mr-2 cursor-pointer" onClick={() => removeSkill(i)}>
                  {s} ✕
                </span>
              ))}
            </div>
            ...
            <div className="md:col-span-3 grid grid-cols-3 gap-3">
              {allExperience.map((exp, i) => (
                <div className="p-3 border rounded-lg" key={i}>
                  ...
                  <button className="btn btn-xs btn-error mt-2" onClick={() => removeExperience(i)}>
                    Remove
                  </button>
                </div>
              ))}
          ...
          <div className="md:col-span-3 grid grid-cols-3 gap-3">
            {[
              ...
              <div key={field.key}>
                ...
                <input className="input w-full" value={experience[field.key]} onChange={(e) => setExperience({ ...experience, [field.key]: e.target.value })} />
              </div>
            ))}

            <div className="col-span-3">
              <label>Description</label>
              <textarea className="textarea w-full" value={experience.jobDesc} onChange={(e) => setExperience({ ...experience, jobDesc: e.target.value })}></textarea>
            </div>
          </div>

          <button className="btn md:col-span-3" onClick={addExperience}>Add Experience</button>
          ...
          <div className="md:col-span-3">
            ...
            <input className="input w-full" value={project.projectTitle} onChange={(e) => setProject({ ...project, projectTitle: e.target.value })} />

            <label className="mt-3 block">Project Description</label>
            <textarea className="textarea w-full" value={project.projectDesc} onChange={(e) => setProject({ ...project, projectDesc: e.target.value })}></textarea>
          </div>

          <button className="btn md:col-span-3" onClick={addProject}>Add Project</button>
        </div>

        <button className="btn btn-primary mt-5" onClick={saveAllInfo}>Save All Info</button>
      </div>
    </div>
  );
}
